...
public class AdvertiserReportsController : Controller
{
    ...
    //Arabic PDF Export with iTextSharp (future)
    //[HttpGet("ExportPdf")]
    //public async Task<IActionResult> ExportPdf([FromQuery] ReportFilterViewModel filter)
    //{
    //    var user = await _userManager.GetUserAsync(User);
    ...
    //    // ---------------------------------------------------------
    //    // 🔥 1) REGISTER ARIAL UNICODE (supports Arabic fully)
    //    // ---------------------------------------------------------
    //    var fontPath = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot/fonts/arial.ttf");
    //    FontFactory.Register(fontPath, "ARIAL_UNI");

    //    // BaseFont for Arabic + Unicode languages
    //    var baseFont = BaseFont.CreateFont(fontPath, BaseFont.IDENTITY_H, BaseFont.EMBEDDED);

    //    // CREATE USABLE FONTS
    //    var titleFont = new Font(baseFont, 20, Font.BOLD, BaseColor.BLACK);
    //    var headerFont = new Font(baseFont, 16, Font.BOLD, new BaseColor(255, 86, 48));
    //    var subHeaderFont = new Font(baseFont, 14, Font.BOLD, new BaseColor(32, 101, 209));
    //    var normalFont = new Font(baseFont, 12, Font.NORMAL, BaseColor.BLACK);
    //    var boldFont = new Font(baseFont, 12, Font.BOLD, BaseColor.BLACK);

    //    // ---------------------------------------------------------
    //    // 🔥 2) TITLE
    //    // ---------------------------------------------------------
    //    var title = new Paragraph("تقرير حملات المعلن", titleFont)
    //    {
    //        Alignment = Element.ALIGN_CENTER,
    //        SpacingAfter = 15
    //    };
    ...
    //    // 🔥 3) ADVERTISER INFO
    //    // ---------------------------------------------------------
    //    var advertiserInfo = new Paragraph(
    //        $"المعلن: {user.UserName}",
    //        new Font(baseFont, 14, Font.BOLD, new BaseColor(255, 86, 48))
    //    )
    //    {
    //        Alignment = Element.ALIGN_CENTER,
    //        SpacingAfter = 20
    ...
    //    AddInfoCell(infoTable, "تاريخ الإنشاء:", DateTime.Now.ToString("yyyy-MM-dd HH:mm"), boldFont, normalFont);
    //    AddInfoCell(infoTable, "فترة التقرير:",
    //        $"{filter?.StartDate?.ToString("yyyy-MM-dd") ?? "N/A"} إلى {filter?.EndDate?.ToString("yyyy-MM-dd") ?? "N/A"}",
    //        boldFont, normalFont);

    //    AddInfoCell(infoTable, "رقم التقرير:", $"ADV-{DateTime.Now:yyyyMMddHHmmss}", boldFont, normalFont);
    ...
    //    // 🔥 5) SUMMARY SECTION
    //    // ---------------------------------------------------------
    //    doc.Add(new Paragraph("ملخص الحملة", headerFont) { SpacingAfter = 10 });

    //    var summaryTable = new PdfPTable(4) { WidthPercentage = 100, SpacingAfter = 20 };
    //    summaryTable.DefaultCell.Padding = 10;
    //    summaryTable.DefaultCell.Border = Rectangle.NO_BORDER;
    //    summaryTable.DefaultCell.BackgroundColor = new BaseColor(255, 247, 245);

    //    var cpc = report.TotalClicks > 0 ? (report.TotalSpend / report.TotalClicks) : 0;
    //    var cpm = report.TotalImpressions > 0 ? (report.TotalSpend / report.TotalImpressions * 1000) : 0;

    //    AddSummaryCard(summaryTable, "إجمالي الإنفاق", $"${report.TotalSpend:N2}", new BaseColor(255, 86, 48));
    //    AddSummaryCard(summaryTable, "عدد النقرات", report.TotalClicks.ToString("N0"), new BaseColor(0, 171, 85));
    //    AddSummaryCard(summaryTable, "CTR معدل النقر", $"{report.CTR:F2}%", new BaseColor(32, 101, 209));
    //    AddSummaryCard(summaryTable, "متوسط CPC", $"${cpc:F2}", new BaseColor(255, 171, 0));
    ...
    //    // 🔥 6) COST ANALYSIS
    //    // ---------------------------------------------------------
    //    doc.Add(new Paragraph("تحليل التكلفة", subHeaderFont) { SpacingAfter = 10 });

    //    var costTable = new PdfPTable(3) { WidthPercentage = 100, SpacingAfter = 20 };
    //    costTable.DefaultCell.Padding = 8;

    //    AddMetricRow(costTable, "CPC تكلفة النقرة", $"${cpc:F2}", normalFont, boldFont);
    //    AddMetricRow(costTable, "CPM تكلفة الألف", $"${cpm:F2}", normalFont, boldFont);
    //    AddMetricRow(costTable, "عدد الظهور", report.TotalImpressions.ToString("N0"), normalFont, boldFont);

    //    doc.Add(costTable);

    //    // ---------------------------------------------------------
    //    // 🔥 7) AD PERFORMANCE TABLE
    //    // ---------------------------------------------------------
    //    if (report.Ads?.Any() == true)
    //    {
    //        doc.Add(new Paragraph("أداء الإعلانات", headerFont) { SpacingAfter = 10 });

    //        var adTable = new PdfPTable(7) { WidthPercentage = 100, SpacingAfter = 20 };
    //        adTable.SetWidths(new float[] { 3, 2, 2, 2, 2, 2, 2 });

    //        AddTableHeader(adTable, new[]
    //        {
    //        "اسم الإعلان", "الحالة", "الظهور", "النقرات", "CTR", "الإنفاق", "CPC"
    //    }, boldFont);
    ...
    //            var statusColor =
    //                ad.Status == AdStatus.Approved ? new BaseColor(0, 171, 85) :
    //                ad.Status == AdStatus.Pending ? new BaseColor(255, 171, 0) :
    //                new BaseColor(145, 158, 171);
    ...
    //    if (report.Regions?.Any() == true)
    //    {
    //        doc.Add(new Paragraph("الأداء الجغرافي", headerFont) { SpacingAfter = 10 });

    //        var regionTable = new PdfPTable(5) { WidthPercentage = 100, SpacingAfter = 20 };
    //        regionTable.SetWidths(new float[] { 2, 2, 2, 2, 2 });

    //        AddTableHeader(regionTable, new[]
    //        {
    //        "الدولة", "المدينة", "الظهور", "النقرات", "CTR"
    //    }, boldFont);
    ...
    //        "تم إنشاء التقرير بواسطة منصة إدارة الإعلانات",
    ...
    // Additional helper method for status-colored rows
